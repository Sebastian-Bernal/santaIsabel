...
watch(formData, (newValue) => {
    ...
    if (formData.value.HistoriaClinica.motivo !== "" && formData.value.Enfermedad.at(-1).valor !== "") {
...
// layout isInternet, validacion formulario, medicamnetos y servicios validar
const traerDatos = () => {
...
const agregarAntecedente = () => {
    ...
    if (ultimoAntecedente === '') {
        $swal.fire({
            position: "top-end",
            text: "Antecedente esta vacio, por favor ingrese un valor",
...
const agregarEnfermedad = () => {
    ...
    if (ultimoAntecedente === '') {
        $swal.fire({
            position: "top-end",
            text: "Efermedad esta vacio, por favor ingrese un valor",
            showConfirmButton: false,
            timer: 1500,
            background: '#d33',
            color: '#fff'
