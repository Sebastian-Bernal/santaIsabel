...
const enviar = async (formData) => {
    ...
    if (estado) {
        ...
        if (respuesta.isConfirmed || respuesta.dismiss) {
            limpiar();
            Object.keys(formData.Empresa).forEach((key) => {
                formData.Empresa[key] = '';
            });
...
<template>
    <div class="flex flex-col bg-white p-4 rounded-2xl gap-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
            <Input v-model="formData.Empresa.nombre" placeholder="Nombre Comercial" name="nombre" minlength="3"
    ...
    <div class="flex flex-col bg-white p-4 rounded-2xl gap-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
            <Input v-model="formData.Empresa.no_identificacion" placeholder="Numero de identificacion" name="IdEmpresa"
                :tamaño="{ 'incompleto': camposVacios && formData.Empresa.no_identificacion === '' }"></Input>
            <Input v-model="formData.Empresa.DV" placeholder="DV" name="DV"
                :tamaño="{ 'incompleto': camposVacios && formData.Empresa.DV === '' }"></Input>
            <Input v-model="formData.Empresa.registroMercantil" placeholder="Registro Mercantil" name="regitroMercantil"
                :tamaño="{ 'incompleto': camposVacios && formData.Empresa.registroMercantil === '' }"></Input>
            <Input v-model="formData.Empresa.direccion" placeholder="Direccion" name="direccion"
                :tamaño="{ 'incompleto': camposVacios && formData.Empresa.direccion === '' }"></Input>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
            <Input v-model="formData.Empresa.pais" placeholder="Pais" name="pais"
                :tamaño="{ 'incompleto': camposVacios && formData.Empresa.pais === '' }"></Input>
            <Input v-model="formData.Empresa.tipoDocumento" placeholder="Tipo de Documento" name="tipodocuemnto"
                :tamaño="{ 'incompleto': camposVacios && formData.Empresa.tipoDocuemnto === '' }"></Input>
            <Input v-model="formData.Empresa.tipoOperacion" placeholder="Tipo de Operacion" name="tipoOperacion"
                :tamaño="{ 'incompleto': camposVacios && formData.Empresa.tipoOperacion === '' }"></Input>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
            <Input v-model="formData.Empresa.tipoEntorno" placeholder="Tipo Entorno" name="tipoEntorno"></Input>
            <Input v-model="formData.Empresa.tipoMoneda" placeholder="Tipo Moneda" name="tipoMoneda"
                :tamaño="{ 'incompleto': camposVacios && formData.Empresa.tipoMoneda === '' }"></Input>
            <Input v-model="formData.Empresa.tipoOrganizacion" placeholder="Tipo de Organizacion"
                name="tipoOrganizacion"></Input>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
            <Input v-model="formData.Empresa.municipio" placeholder="Municipio" tipo="municipio"
                name="municipio"></Input>
            <Input v-model="formData.Empresa.tipoResponsabilidad" placeholder="Tipo de Responsabilidad"
                name="tipoResponsabilidad"></Input>
            <Input v-model="formData.Empresa.tipoRegimen" placeholder="Tipo de Regimen" name="tipoRegimen"
                :tamaño="{ 'incompleto': camposVacios && formData.Empresa.tipoRegimen === '' }"></Input>
        </div>
        <div class="w-full flex justify-end">
            <button @click="varView.formComplete ? enviar(formData) : validarform()"
                class="bg-blue-500 cursor-pointer hover:bg-blue-600 text-white text-sm p-3 rounded-2xl flex items-center gap-3">
